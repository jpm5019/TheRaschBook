...
This chapter provides a basic overview of the Many-Facet Rasch Model (MFRM; -[] Cheng please add reference to Linacre, 1989), along with guidance for analyzing data with the MFRM using R. We use the TAM package (-[] Cheng please add reference to Robitzsch et al., 2020) for all of the analyses in this chapter. In the first example, we demonstrate how to apply the MFRM to multi-faceted data that are stored in *wide format* (one row for each subject). Then, we demonstrate how to apply the MFRM to multi-faceted data that are stored in *long format* (multiple rows for each subject). After the analyses are complete, we present an example description of the results from one example. The chapter concludes with a challenge exercise.
...
In Equation 6.1, $\theta$ is person ability and $\tau_k$ is the rating scale category threshold, which can be modified to reflect the PC model as needed. $\Sigma$ facets $\epsilon$ is a linear combination of facets that are specific to each modeling context. For example, facets for MFR model analysis of a performance assessment could include raters and domains. According to Equation 6.1, the probability for an observation in category *k*, rather than in category *k*- 1 for Person *n* is modeled as the difference between Person n's location, the location of the researcher-specified facets, and the difficulty associated with providing a response in category *k*.
...
```{r results='hide'}
RS_MFR_model <- tam.mml.mfr(resp = ratings, facets = facets, formulaA = style_RS_MFRM, pid = students, verbose = FALSE)

summary(RS_MFR_model)
```

The summary of the MFRM is lengthy. Included among the output are several details that may be important for some researchers, including details about each iteration, global model-fit indicators (e.g., deviance,  log-likelihood, AIC, BIC) and an estimate of person separation reliability (EAP Reliability). 
...
RS_MFRM_rater.summary.results <- rbind(mean(centered.rater.locations_RSMFR),
                            sd(centered.rater.locations_RSMFR),
...
# Example 2:  Running the Many-Facet Rasch Model for Long-Format Data using the TAM Package (Partial Credit MFRM)
...
## Partial Credit MFRM Analysis with Long-Format Data
...
# specify the PC-MFR model:
PC.writing.formula <- ~ item + (rater * step)
...
Because we used a PC formulation of the MFRM, the thresholds were estimated separately for each rater. We save the facet estimates for domains and raters as we did in the first example. However, for the threshold estimates, we extract the values labeled "step:rater".
...
# Extract rater-specific threshold estimates:
threshold.estimates <- subset(facet.estimates, facet.estimates$facet == "step:rater")
```

### Domain facet results

We will center the domain parameter estimates for ease of interpretation, as we have done previously with item estimates. Then, we will request a summary of the centered domain locations.

```{r}
uncentered.domain.locations_PCMFR <- domain.estimates$xsi
centered.domain.locations_PCMFR <- scale(uncentered.domain.locations_PCMFR, scale = FALSE)
summary(centered.domain.locations_PCMFR)
```

From the summary of domain locations, we can see that the domain difficulty estimates range from -0.90 for the easiest domain to 0.67 for the most difficult domain.

### Rater subgroup facet results
We do not need to adjust the location of the raters to account for centering the domain facet because the facet locations are already centered at zero logits. We will examine the rater estimates using the *summary()* function.
```{r}
summary(rater.estimates$xsi)
```

The rater location estimates from this model range from -0.54 to 0.76.

### Threshold estimates
Because we used a PC formulation of the MFRM, we have separate estimates of the rating scale category thresholds for each rater. In the following code, we organize the threshold values in a matrix with separate rows for each rater. Then, we print the first six rows of the threshold matrix to the console to preview it.
 
```{r}
n.raters <- nrow(rater.estimates)
n.thresholds <- max(writing.resp)

rater_taus <- matrix(data = NA, nrow = n.raters, ncol = n.thresholds)

for(rater.number in 1:n.raters){
  
  rater.threshold.labels <- NULL
  for(s in 1:n.thresholds){
   
    if(rater.number < 10) rater.threshold.labels[s] <- paste("step", s, ":rater_", rater.number, "_", sep = "")
    
        if(rater.number >= 10) rater.threshold.labels[s] <- paste("step", s, ":rater_", rater.number, sep = "")
    
  }
    rater.thresholds <- subset(threshold.estimates, 
                           threshold.estimates$parameter == rater.threshold.labels)
    
    rater.thresholds.t <- t(rater.thresholds$xsi)
  
  rater_taus[rater.number,] <- rater.thresholds.t[1,]
}

rater_taus <- cbind.data.frame(c(1:n.raters), rater_taus)
names(rater_taus) <- c("rater", "t1", "t2", "t3")
...
To interpret the student locations in the same frame of reference as our centered domain locations, we need to subtract the original (uncentered) domain mean location from the student locations:
```{r}
student.locations_PCMFR$theta_adjusted <- (student.locations_PCMFR$theta - mean(uncentered.domain.locations_PCMFR))
summary(student.locations_PCMFR$theta_adjusted)
```

From the summary of the adjusted student achievement locations, we can see that student achievement ranges from -7.30 logits for the student with the lowest achievement estimate to 7.75 for the student with the highest achievement estimate. On average, the students were located higher (M = 0.72) than the average domain location (M = 0.00).

### Plot a Wright Map

Next, we will plot a Wright Map to display the locations of the parameter estimates for our PC-MFR model. To do this, we need to manipulate the location estimate objects into the format required for the WrightMap package. 

The *wrightMap()* function requires us to store the rater location estimates as a matrix that shows rater-specific threshold locations. We already did this when we created the *rater_taus* object earlier. 

Next, we need to store the adjusted student locations and the domain locations in a single data frame. Doing so will allow us to plot the locations for students and domains in the same Wright Map. We store these values in a data frame object called *persons*.
```{r}
persons.domains <- cbind.data.frame(student.locations_PCMFR$theta_adjusted,
                            domain.estimates$xsi)
```

Finally, we can plot the Wright map using the *wrightMap()* function. We specify several graphical parameters to modify the appearance of the plot.
```{r}
wrightMap(thetas = persons.domains,
          axis.persons = "Students",
          dim.names = c("Students", "Domains"), 
          thresholds = rater_taus[,-1],
          show.thr.lab	= TRUE,
          label.items.rows= 2,
          label.items = rater.estimates$parameter,
          axis.items = "Raters",
...
In this *Wright Map* display, the results from the PC-MFRM analysis of the writing assessment ratings are summarized graphically using the same format as the first example in this chapter. The major differences between the models is that domains are included as a facet instead of student subgroups, and each rater has a unique set of threshold estimates.
...
The resulting residual matrix (*resid.matrix*) contains unstandardized residuals for each rater in combination with each domain and each student. Next, we will calculate standardized residuals and save them in a matrix.
```{r}
std.resid.matrix <- as.data.frame(resids$stand_residuals)
```

The resulting standardized residual matrix (*std.resid.matrix*) contains standardized residuals for each rater in combination with each domain and each student. Next, we will calculate the variance in observations due to Rasch-model-estimated locations:
```{r}
# Variance of the observations: VO
...
Our analysis indicates that approximately 71.61% of the variance in ratings can be explained by the PC-MFRM estimates of student, domain, and rater locations on the logit scale that represents the latent variable. 
...
On average, the MSE outfit and infit statistics are slightly lower than the expected value of 1 (Mean Outfit = 0.96, Mean Infit = 0.97). The average values of the standardized fit statistics are also slightly lower than their expected value of 0 (Mean Std. Outfit = -0.26, Mean Std. Infit = -0.21). For both the standardized and unstandardized fit statistics, there is notable variability across the student sample. This result suggests that model-data fit varies for individual students.
...
As needed, researchers can also examine model-data fit statistics specific to levels of explanatory facets, such as domains in the case of the current example. This can be accomplished by calculating summaries of fit statistics within domains. The code below  calculates averages of rater fit within each domain. For a more detailed view of rater fit within domains, we create an object with domain-specific rater fit statistics in the subsequent code block.
...
PC_MFRM_student.summary.results <- rbind(mean(student.locations_PCMFR$theta_adjusted),
                              sd(student.locations_PCMFR$theta_adjusted),
...
names(PC_MFRM_Table2) <- c("Rater ID", "Average Rating", "Rater Location","Rater SE",
                           "Threshold 1", "Threshold 2", "Threshold 3", names(rater.fit_results[, -1]))                           
...
Table 3 is a table that summarizes the overall calibrations of individual domains For data sets with manageable sample sizes such as the writing assessment example in this chapter, we recommend reporting details about each element of explanatory facets (e.g., individual domains) in a table similar to this one.
...
PC_MFRM_Table4 <- cbind.data.frame(rownames(student.locations_PCMFR),
                          Person_Avg_Rating,
                          student.locations_PCMFR$theta_adjusted,
...
# Notes on formulations for MFR Models
...
Table 6.1 provides an overview of some popular MFRM formula specifications for the TAM package that may be useful for analyses that differ from those used in this chapter. For example, analysts can specify a dichotomous MFRM by omitting "step" from the formula. In addition, analysts can specify an interaction between facets using the multiplication symbol (*) to create interaction terms.

-[] Cheng, please add Table 6.1 about here.

# Example results section based on Example 1:
