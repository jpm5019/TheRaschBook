...
We will also use the "psych" package (Revelle, 2016) to perform a principal components analysis (PCA) of Rasch model standardized residual correlations. To get started with the psych package, view the citation information, and then install and load it into your R environment using the following code:
```{r}
citation("psych")
...
dichot.transreas_MMLE$WLEreliability
...
## Rasch Model Fit Analysis with Joint Maximum Likelihood Estimation in TAM

Finally, we demonstrate model-data fit analyses using the TAM package (Robitzsch, Kiefer, & Wu, 2020) with Joint Maximum Likelihood Estimation (JMLE). As in the previous section, we do not provide detailed annotations or discussions of the results from these analyses because they are quite similar to those that we have already presented in this chapter.

### Run the Rasch Model
```{r results='hide'}
dichot.transreas_JMLE <- tam.jml(transreas.responses)
```
...
Linacre, John Michael. "Structure in Rasch residuals: Why principal components analysis (PCA)?" *RaschMeasurement Transactions*, 12(2),1998, 636.

Linacre, John Michael. "Data variance: Explained, modeled and empirical." *Rasch Measurement Transactions*, 17(3), 2003, 942–943.
...
Rasch, Georg. *Probabilistic models for some intelligence and achievement tests(Expanded edition, 1980)*. University of Chicago Press, 1960.
...
William, Revelle. *psych: Procedures for Psychological, Psychometric, and Personality Research.* Northwestern University, Evanston, Illinois. R package version 2.0.12, 2020. https://CRAN.R-project.org/package=psych.
