...
The results indicate that $\tau_1$ <= $\tau_2$ is true for all items except Item 13. The disordered thresholds for Item 13 indicate that lower locations on the construct were required to provide a rating in category 1 compared to category 2, when the opposite order would have been expected given the rating scale category labels. This result suggests that additional research may be required to understand students' responses to Item 13. For Item 12, the expression returns a value of "NA" because only one threshold was estimated for this item.
...
To supplement the numeric evaluation of rating scale category ordering, analysts can examine plots of rating scale category probabilities for evidence that the rating scale categories are ordered as expected. Specifically, the probabilities associated with each item should increase and decrease in the expected order as locations on the latent variable progress from low to high. In addition, analysts can examine these plots for evidence that each category describes a unique range of locations on the latent variable. In other words, each category should be the most-probable response for some range of locations on the latent variable. Distinct category probability curves provide evidence that the rating scale categories can distinguish among examinees with different locations on the latent variable. In cases where such distinctions are not observed, researchers may consider revising the scale to include fewer categories prior to further analysis, as well as making revisions to the scale length or category labels prior to future administrations. For details about these procedures and guidance in decision-making related to rating scale revisions, we recommend that readers consult Bond and Fox (2020, chapter 11), as well as Linacre (2002).
...
Now we can run the PCM with our design matrix using the *tam.mml()* function with several specifications. After we run the model, we will request a summary of the model results using the *summary()* function.
-[done] omit the model run output, keep the summary output.
...
-[Done] omit the MMLE_fit output, keep the summary output.
...
## PCM Application with Joint Maximum Likelihood Estimation
...
```
The *tam.fit()* function provides mean square error (*MSE*) and standardized (*t*) versions of the infit and outfit statistics for Rasch models for each item estimate.
...
## Bibliography
Andrich, David. "A rating formulation for ordered response categories." *Psychometrika, 43* No.4(1978): 561–573. https://doi.org/10.1007/BF02293814
...
Linacre, John M. “Optimizing Rating Scale Category Effectiveness.” Journal of Applied Measurement 3, no. 1 (2002): 85–106.
...
Masters, Geoff. "A Rasch model for partial credit scoring." *Psychometrika, 47* No.2, (1982): 149–174. https://doi.org/10.1007/BF02296272

Mellenbergh, Gideon. "Conceptual Notes on Models for Discrete Polytomous Item Responses." *Applied Psychological Measurement, 19* No.1 (1995), 91–100. https://doi.org/10.1177/014662169501900110
